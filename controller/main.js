...
app.controller('chatWindow', ['$scope', 'DataStream', function ($scope, DataStream) {
    ...
    const makeIncidentTable = function (data) {
        ...
        data.forEach(d => {
            html = html + '<tr><td class="incident" style="cursor:pointer" onclick="showDetails(\''+d.incidentRef+'\')">' + d.incidentRef + '</td><td>' + d.priority + '</td><td>' + truncate(d.incidentDesc) + '</td></tr>';
