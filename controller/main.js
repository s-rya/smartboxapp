...
app.controller('chatWindow', ['$scope', 'DataStream', function ($scope, DataStream) {
    ...
    let scripts = '<script>if (typeof module === \'object\') {window.module = module;module = undefined;}</script>' +
        ...
        '<script>$(document).ready(function(){$("div[id^=\'block\']").click(function(){let divID = $(this).attr(\'id\');if($("#"+divID + "feedback").is(":visible")){$(this).css("height", "85px");$("#"+divID + "feedback").hide();} else {$(this).css("height", "auto");$("#"+divID + "feedback").show();}for(i=0;i<10;i++){if(divID!="block"+i)$("#block"+i).css(\'height\',\'85px\');if(divID+"feedback"!="block"+i+"feedback"){$("#block"+i+\'feedback\').hide();}}});});</script>' +
